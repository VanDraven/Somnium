package lucyds {
	...
	public class Monster extends FlxSprite
	{
		...
		{
			chemin = new FlxPath;
			//var chemin2 : FlxPath =  PlayState.groundMap.findPath(this.getMidpoint(null), PlayState.lucie.getMidpoint(null));
			//trace(chemin2);
			chemin.addPoint(PlayState.lucie.getMidpoint(null), false);
			this.followPath(chemin, 275);
			...
			if(distance <= flicker)
			{
				...
				PlayState.parallax.scrollFactor.x = PlayState.parallax.scrollFactor.y = 1.5;
			}
			else
			{
				PlayState.parallax.scrollFactor.x = PlayState.parallax.scrollFactor.y = 0.5;
