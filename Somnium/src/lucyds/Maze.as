package lucyds {
	...
	{		
		...
		public var numCasesW	: uint = 90;
		public var numCasesH	: uint = 60;
		
//		public var tempsDest		: Number 						= 0.15 / PlayState.dByFloor;
		public var tempsDest		: Number;
		...
		public var fieldValues 		: Vector.<Vector.<uint>>		= new Vector.<Vector.<uint>>(numCasesW);
		...
		public var firstRealBN		: int;
		public var pathRatio 		: Number;
		...
		public function Maze():void
		{
			...
			pathRatio = realBN() / numFields;
			
			if(PlayState.gameSave.data.tropTemps > 0 && !PlayState.gameSave.data.dByFloor)
				tempsDest = PlayState.gameSave.data.lastTempsDest - (PlayState.gameSave.data.tropTemps / (realBN() / pathRatio));
			else if(PlayState.gameSave.data.tropTemps > 0 && PlayState.gameSave.data.dByFloor)
				tempsDest = PlayState.gameSave.data.lastTempsDest + (PlayState.gameSave.data.tropTemps / (realBN() / pathRatio));
			else
				tempsDest = 0.2;
			...
			trace("pathRatio : ", pathRatio);
			trace("tropTemps : ", PlayState.gameSave.data.tropTemps);
			trace("tpsDest : ", tempsDest);
		...
		{			
			...
			var tempsDestFin : Number = (tempsDest - PlayState.jump*0.03);
                                             ...
                                             endY    : int) : void 
		{
            ...
            var v : Vector.<Object> = new <Object>[{
                x: startX,
                y: startY,
                triedPaths: 0
            }];
           
            fields[v[0].x][v[0].y] = 1;
           
            while (v.length > 0 && (v[0].x - endX) * (v[0].x - endX) + (v[0].y - endY) * (v[0].y - endY) > 1)
            {
                if (v[0].triedPaths == 15) {
                    v.shift();
                    continue;
                }
                ...
                if (randomisedNumber < 0.25) {
                    v[0].triedPaths |= 1;
                    v.unshift({x: v[0].x + 1, y: v[0].y, triedPaths: 0});
                }
                else if (randomisedNumber < 0.5) {
                    v[0].triedPaths |= 2;
                    v.unshift({x: v[0].x, y: v[0].y + 1, triedPaths: 0});
                }
                ...
                if (v[0].x < 1 || v[0].x >= numCasesW - 1|| v[0].y < 1 || v[0].y >= numCasesH - 1 || // test bords
                    fields[v[0].x][v[0].y] == 1 || // test case courante
                    !testAdjacents(v[0].x, v[0].y) || // test adjacents
                    !testDiagonals(v[0].x, v[0].y)) {// test diags
		...
		private function defineStart() : void
		{
			...
			{
				startX = 1;
				startY = (FlxG.random()* (numCasesH - 3)) + 1;
			}
			else if (rand < 0.5)
			{
				startX = numCasesW - 2;
			...
			{
				startY = 1;
				startX = (FlxG.random()* (numCasesW - 3)) + 1;
			}
			else
			{
				startY = numCasesH - 2;
		...
		{					
			...
			{
				endX = numCasesW - 2;
				endY = (FlxG.random()* (numCasesH - 3)) + 1;
			}
			...
			{
				endY = numCasesH - 2;
				endX = (FlxG.random()* (numCasesW - 3)) + 1;
			}
